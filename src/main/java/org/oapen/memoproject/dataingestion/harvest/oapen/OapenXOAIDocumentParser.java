 ...
 *
 */
public final class OapenXOAIDocumentParser implements EntitiesSource {
	
	private final Element element;
	private final XPath xpath;

	
	public OapenXOAIDocumentParser(Element element) {
	...
	private static final Logger logger = 
			LoggerFactory.getLogger(OapenXOAIDocumentParser.class);
	...
	public Set<Classification> getClassifications() {
		...
		getNodeList(path).ifPresent(nodes -> {
			...
			classifications.addAll(OapenXOAIDocumentParserUtils.parseClassifications(lines));
		}); 
	 ...
	 * (Currently - August 2025 - these will not be available for OAPEN)
	 */
	...
	private Set<Identifier> getIdentifiers() {
		...
		getNodeList(pathOCN1).stream().forEach(nodes -> {
			...
			usedIds.addAll(ocns);
			ocns.forEach(ocn -> identifiers.add(new Identifier(OapenXOAIDocumentParserUtils.parseOCN(ocn),"OCN")));
		});
		...
		getNodeList(pathISBN1).stream().forEach(nodes -> {
			Set<String> isbns = OapenXOAIDocumentParserUtils.parseISBNOrISSN(getTextValueSet(pathISBN1));
		...
		getNodeList(pathISBN2).stream().forEach(nodes -> {
			Set<String> isbns = OapenXOAIDocumentParserUtils.parseISBNOrISSN(getTextValueSet(pathISBN2));
			usedIds.addAll(isbns);
			isbns.forEach(isbn -> identifiers.add(new Identifier(isbn,"ISBN")));
		});

		getNodeList(pathISSN).stream().forEach(nodes -> {
			Set<String> issns = OapenXOAIDocumentParserUtils.parseISBNOrISSN(getTextValueSet(pathISSN));
	...
	private Set<String> getLanguages() {
		...
		return OapenXOAIDocumentParserUtils.parseLanguages(getTextValueSet(path));
	}
	...
	private Set<String> getSubjectsOther() {
		...
		if (!q.isEmpty()) return OapenXOAIDocumentParserUtils.parseSubjects(q);
		else return q;
	}
		...
		for (String path: paths) {
			...
			getNodeList(path).ifPresent(nodes -> {
				...
				for (int i=0; i < nodes.getLength(); i++) {
					...
					OapenXOAIDocumentParserUtils.yearFromString(datetext).ifPresent(years::add);
				}	
			});
		}
	...
	private Optional<String> getHandle() {
		...
		// Monadish isn't it?
		return getTextValue(path).flatMap(n -> OapenXOAIDocumentParserUtils.extractHandleFromIdentifier(n));
	}
	...
	private Optional<LocalDate> getAvailableDate() {
		...
		if (!q.isEmpty()) return OapenXOAIDocumentParserUtils.parseDate(q.get());
		else return Optional.empty();
	}
	...
	private Optional<LocalDate> getAccessionedDate() {
		...
		return dateStrings.stream()
			.map(OapenXOAIDocumentParserUtils::parseDate)
	...
	private Optional<Integer> getIssuedYear() {
		...
		if (!q.isEmpty()) return OapenXOAIDocumentParserUtils.yearFromString(q.get());
		else return Optional.empty();
	}
